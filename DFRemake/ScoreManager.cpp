...
MiceWave* ScoreManager::generateWave(int numWave) {
	...
	for (int i = 0; i < 3; i++) {
		...
		attacks[i] = new MiceAttack(numWave + i, numWave < 7 ? 8 - numWave : 2, curr_wave / 5, 0.05 * (curr_wave/3));
	}
	...
	MiceWave* generatedWave = new MiceWave(attacks, numWave, numWave < 5 ? 8 - numWave : 4, curr_wave / 5, 0.05 * (curr_wave / 3));
